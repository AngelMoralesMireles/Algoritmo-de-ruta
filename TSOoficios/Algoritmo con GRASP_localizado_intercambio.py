...
df_dia2 = df_dia2[df_dia2["TiempoEstimado"] > 0]  # Excluir tiempos cero

# Asignar valor numérico por prioridad
df_dia2["Valor"] = df_dia2["Prioridad"].apply(lambda p: 3 if p == "Alta" else (2 if p == "Media" else 1))

# Capacidad máxima de minutos disponibles
capacidad_maxima = 480

# Algoritmo GRASP + 2-intercambio (prioriza alta prioridad primero)
def grasp_2intercambio(df, capacidad, iteraciones=500):
    ...
    for _ in range(iteraciones):
        # Ordenar por prioridad más alta y menor tiempo estimado
        df_ordenado = df.sort_values(by=["Valor", "TiempoEstimado"], ascending=[False, True]).reset_index(drop=True)
        ...
        # 2-intercambio para mejora local
        improved = True
        while improved:
            improved = False
...
# Mostrar conteo de prioridades en entregas realizadas
